...
return {
    ...
    config = function()
        ...
        -- NOTE: find current path of python
        -- python -c "import sys; tp=sys.executable; print(tp.replace('\\', '/'))"
        python_code = "import sys; tp=sys.executable; print(tp.replace('\\\\', '/'))"
        python_cmd = "python -c \"" .. python_code .. "\""
        local handle = io.popen(python_cmd)
        condaPath = handle:read("*a")
        handle:close()
        condaPath = condaPath:gsub("\n", "")

        dap.adapters.python = {
            type = 'executable';
            -- command = 'D:/Software/Miniconda/python.exe';
            command = condaPath;
            -- command = tostring(pythonPath);
            -- command = os.getenv("VIRTUAL_ENV") .. "D:/Software/Miniconda/python.exe";
        ...
        dap.configurations.python = {
            {
                ...
                --     else
                --         return 'D:/Software/Miniconda/python.exe'
                --     end
                -- end;
            },
        }
        -- NOTE: Way 2: dap-nvim-python
    ...
    keys = {
        { "<leader>dR", "<cmd>!python %<CR>", "n", { noremap = true, silent = true } },
